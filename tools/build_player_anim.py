#!/usr/bin/env python3
"""Author locomotion clips onto the Captain Clark Mixamo rig and inject them into a GLB.

The player model ships with no animation. We synthesise Idle/Walk/Run/Jump/Fall using
'aim based' posing: each bone is pointed in a target world-space direction and we solve
the local quaternion through the rest hierarchy. Because we cannot render, every clip is
validated with forward kinematics (feet below hips, alternating stride, knees flex up).
"""
...
RESTW=fk({})
def B(s):
    for nm,i in byname.items():
        if nm.startswith("mixamorig:"+s): return i
    raise KeyError(s)
J={k:B(k) for k in["Hips","Spine","Spine1","Spine2","Neck","Head",
...
def rest_dir(name):  # world rest direction of a bone (joint->child)
    j=J[name]; c=J[CHILD[name]]
    return norm(RESTW[c][:3,3]-RESTW[j][:3,3])
def rest_world_q(name):
    R=RESTW[J[name]][:3,:3].copy()
    for k in range(3): R[:,k]/=(np.linalg.norm(R[:,k])+1e-12)
    return mat2quat(R)
def parent_world_q(name):
    p=parent[J[name]]; R=RESTW[p][:3,:3].copy()
    for k in range(3): R[:,k]/=(np.linalg.norm(R[:,k])+1e-12)
    return mat2quat(R)

def aim_local(name, world_dir):
    """local quaternion so the bone points along world_dir (rest parent frame)."""
    qw=from_two(rest_dir(name), world_dir)
    desired_world=qmul(qw, rest_world_q(name))
    return qmul(qconj(parent_world_q(name)), desired_world)
def offset_local(name, world_axis, ang):
    """rotate a bone about a world-space axis by ang, expressed as local quaternion."""
    j=J[name]; R=RESTW[j][:3,:3]; local_axis=R.T@np.array(world_axis,float)
...
# ---- pose builder: returns {node_index: quaternion} for one frame ----
def base_arm_dirs():
    # lowered, slightly forward & outward
    return {"LeftArm":norm([0.30,-1.0,0.05]),"RightArm":norm([-0.30,-1.0,0.05])}
def pose(clip, t, dur):
    p=2*math.pi*(t/dur)
    q={}
    arm=base_arm_dirs()
    def setaim(name,d): q[J[name]]=aim_local(name,d)
    def setoff(name,axis,a): q[J[name]]=offset_local(name,axis,a)

    if clip=="Idle":
        br=math.sin(p)             # breathing
        sway=math.sin(p*0.5)
        setoff("Spine",[1,0,0],math.radians(2+1.2*br))
        setoff("Spine1",[1,0,0],math.radians(1.5))
        setoff("Spine2",[0,1,0],math.radians(2*sway))
        setoff("Neck",[1,0,0],math.radians(-3))
        setoff("Head",[0,1,0],math.radians(2.5*sway))
        setoff("Hips",[0,0,1],math.radians(1.5*sway))
    ...
    if clip in("Walk","Run"):
        ...
        s=math.sin(p)
        # legs (aim thigh fwd/back about worldX; foot fwd at -Z)
        thL=math.radians(legA)*s; thR=-thL
        setaim("LeftUpLeg",  rotv([0,-1,0],[1,0,0], thL))
        setaim("RightUpLeg", rotv([0,-1,0],[1,0,0], thR))
        # knee flex: bend when that leg swings back / lifts (max around back phase)
        kL=math.radians(knee)*max(0.0,-math.sin(p-0.6))
        kR=math.radians(knee)*max(0.0,-math.sin(p+math.pi-0.6))
        setaim("LeftLeg",  rotv(rotv([0,-1,0],[1,0,0],thL),[1,0,0], kL))
        setaim("RightLeg", rotv(rotv([0,-1,0],[1,0,0],thR),[1,0,0], kR))
        # feet counter to stay flat-ish
        ...
        # torso: forward lean + counter twist + bob
        setoff("Spine",[1,0,0], math.radians(lean))
        setoff("Spine1",[0,1,0], math.radians(6)*(-s))
        setoff("Spine2",[0,1,0], math.radians(4)*(-s))
        setoff("Neck",[1,0,0], math.radians(-lean*0.6))
        setoff("Hips",[0,1,0], math.radians(7)*s)
        bobv=bob*(1-math.cos(2*p))*0.5
        return q, bobv

    if clip=="Jump":
        ph=t/dur
        crouch=math.sin(min(1,ph*1.4)*math.pi)   # crouch then extend
        th=math.radians(55)*crouch
        setaim("LeftUpLeg", rotv([0,-1,0],[1,0,0], th))
        setaim("RightUpLeg",rotv([0,-1,0],[1,0,0], th))
...
# ---- sample + validate a clip ----
def sample(clip, dur, fps, loop):
    n=max(2,int(round(dur*fps))+(0 if loop else 1))
    times=[]; frames=[]; bobs=[]
    for k in range(n):
        t=(k/(n-1))*dur if not loop else (k/n)*dur
        q,bob=pose(clip,t,dur); times.append(t); frames.append(q); bobs.append(bob)
    if loop: times.append(dur); frames.append(frames[0]); bobs.append(bobs[0])
    return times,frames,bobs
def validate(clip,times,frames):
    hipY=RESTW[J["Hips"]][1,3]
    minLF=1e9;maxLFb=-1e9; alt=[]
    for q in frames:
        W=fk({i:(lambda L,qq: setrot(L,qq))(REST[i].copy(),q[i]) for i in q})
        lf=W[J["LeftFoot"]][:3,3]; rf=W[J["RightFoot"]][:3,3]
        minLF=min(minLF,lf[1]); maxLFb=max(maxLFb,lf[1]); alt.append(lf[2]-rf[2])
    feet_below=maxLFb<hipY
    strides=max(alt)-min(alt)
    print("  [%s] feet_below_hip=%s left_foot_y=[%.2f..%.2f] stride_sep=%.3f"%(clip,feet_below,minLF,maxLFb,strides))
    return feet_below
def setrot(L,q):
    s=np.array([np.linalg.norm(L[:3,0]),np.linalg.norm(L[:3,1]),np.linalg.norm(L[:3,2])])
    L[:3,:3]=qmat(q)@np.diag(s); return L

# ---- glTF buffer append helpers ----
def pad4():
    while len(blob)%4: blob.append(0)
def add_view(data):
    pad4(); off=len(blob); blob.extend(data)
    bv=BufferView(buffer=0,byteOffset=off,byteLength=len(data)); g.bufferViews.append(bv); return len(g.bufferViews)-1
def add_time(times):
    data=struct.pack('<%df'%len(times),*times); bv=add_view(data)
    a=Accessor(bufferView=bv,componentType=5126,count=len(times),type="SCALAR",min=[float(min(times))],max=[float(max(times))])
    g.accessors.append(a); return len(g.accessors)-1
def add_vec(vals,dim):
    flat=[]; 
    for v in vals: flat.extend([float(x) for x in v])
    data=struct.pack('<%df'%len(flat),*flat); bv=add_view(data)
    a=Accessor(bufferView=bv,componentType=5126,count=len(vals),type=("VEC3" if dim==3 else "VEC4"))
    g.accessors.append(a); return len(g.accessors)-1
...
for name,dur,fps,loop in CLIPS:
    ...
    for nd in sorted(nodes):
        ...
        out=add_vec(vals,4)
        si=len(anim.samplers); anim.samplers.append(AnimationSampler(input=tin,output=out,interpolation="LINEAR"))
        anim.channels.append(AnimationChannel(sampler=si,target=AnimationChannelTarget(node=nd,path="rotation")))
    # hips vertical bob -> translation channel (convert world Y to hips-parent local)
    if any(abs(b)>1e-6 for b in bobs):
        base=np.array(g.nodes[J["Hips"]].translation or [0,0,0],float)
        tr=[]
        for b in bobs:
            off=hips_parent_R.T@np.array([0,b,0]); tr.append(base+off)
        out=add_vec(tr,3)
        si=len(anim.samplers); anim.samplers.append(AnimationSampler(input=tin,output=out,interpolation="LINEAR"))
