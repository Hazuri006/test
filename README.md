...
> the hospital is dressed with a **modular GLB prop pack**; everything else uses
> **procedural fallback geometry** so the game still runs even if those assets are
> removed. See [Replacing fallback assets](#installing--replacing-fallback-assets)
> to drop in more, and `ATTRIBUTION.md` to record asset licences.
...
- Procedural geometry, materials and audio are **original content** generated by this
  project. The bundled GLB monster and hospital prop pack are **user‑supplied
  external assets** — verify and record their licences in `ATTRIBUTION.md` before
  distributing.
